    ...
    public partial class GCodeFromImage
    {
        ...
        private void GenerateGCodeHorizontal(bool withToolTable, bool applyPixelArt)
        {
            ...
            if (withToolTable)
            {
             //   ToolTable.SortByPixelCount(false);      // sort by color area (max. first)
                ...
                { ConvertColorMapGraphic(resolX); useGraphics = true; }       	// generate GCode from coleccted pixel positions
            }
            else
            {
            ...
            Gcode.PenUp(finalString, "PU");                             // pen up
        ...
        private void ConvertColorMap(float resol)
        {
            ...
            for (int index = 0; index < Colors.MyPalette.Count; index++)  	// go through lists
            {
                ...
                if (colorMap.ContainsKey(toolNr) && Colors.MyPalette[index].Use)
                {
                 ...
                 //   if (useResoAsPenWidth) 
                        penWidth = resoOutlineX;
                 //   else penWidth = ToolTable.IndexWidth();
                    ...
                    Gcode.Comment(finalString, string.Format("{0} Id=\"{1}\" PenColor=\"{2}\" PenName=\"{3}\" PenWidth=\"{4:0.00}\">", XmlMarker.GroupStart, (++pathCount), ColorTranslator.ToHtml(Colors.MyPalette[index].Col).Substring(1), Colors.MyPalette[index].Name, penWidth));		//toolTable.indexName()));
                    Gcode.ReduceGCode = false;
                    Gcode.PenUp(finalString, "PU"); // start
                    //                    gcode.MoveToRapid(finalString, 0, 0);          // move to start pos
                    Gcode.ReduceGCode = true;

                    tmp += Colors.MyPalette[index].Name + "\r\n";
                    ...
                    if (cBGCodeOutline.Checked)
                    {
                        ...
                        if (!cBGCodeOutlineSmooth.Checked)
                            smoothCnt = 0;
                        outlineList = Vectorize.GetPaths(resultToolNrArray, adjustedImage.Width, adjustedImage.Height, (short)toolNr, smoothCnt, (float)0.5 / resoOutlineX, cBGCodeOutlineShrink.Checked);// half pen-width in pixels
                        int cnt = 0;
                        float tmpY;
                        foreach (List<PointF> path in outlineList)
                        {
                            if (path.Count > 0)
                            {
                                ...
                                tmpY = (adjustedImage.Height - 1) - tmpP.Y; // start point
                                                                            //    Gcode.Comment(finalString, string.Format("{0} ToolNr=\"{1}\">", XmlMarker.ContourStart, cnt));
                                Gcode.Comment(finalString, string.Format("{0} Id=\"{1}\" Geometry=\"Contour\" PenColor=\"{2}\" PenWidth=\"{3:0.00}\">", XmlMarker.FigureStart, cnt, ColorTranslator.ToHtml(Colors.MyPalette[index].Col).Substring(1), penWidth));
                                Gcode.MoveToRapid(finalString, tmpP.X * resoOutlineX, tmpY * resoOutlineY, "");          // move to start pos
                                Gcode.PenDown(finalString, "PD");// " contour "+cnt);
                    ...
                    if (cBGCodeFill.Checked)
                    {
                        ...
                        Gcode.Comment(finalString, string.Format("{0} Id=\"{1}\" Geometry=\"Fill\" PenColor=\"{2}\" PenWidth=\"{3:0.00}\">", XmlMarker.FigureStart, pathCount, ColorTranslator.ToHtml(Colors.MyPalette[index].Col).Substring(1), penWidth));
                        if (shrink > 0)
                        {
        ...
        /// process color map and generate gcode
        /// </summary>
        private void ConvertColorMapGraphic(float resol)
        {
            Logger.Info("▼▼  ConvertColorMap Graphic");
            int toolNr, skipTooNr = 1;
            short key;
            List<List<PointF>> outlineList;// = new List<List<Point>>();
            PointF tmpP;
            ...
            {
                toolNr = Colors.MyPalette[index].ToolNr;
                toolColor = ColorTranslator.ToHtml( Colors.MyPalette[index].Col).Substring(1);

                if (colorMap.ContainsKey(toolNr) && Colors.MyPalette[index].Use)
                {
                    Graphic.SetPenColor(toolColor);
                    Graphic.SetPenFill(toolColor);
                    Graphic.SetPenWidth(resoOutlineX.ToString());
                    Graphic.SetGeometry("Outline");
                    ...
                    if (cBGCodeOutline.Checked)
                    {
                        ...
                        foreach (List<PointF> path in outlineList)
                        {
                            if (path.Count > 0)
                            {
                                ...
                                startX = tmpP.X * resoOutlineX;
                                startY = tmpY * resoOutlineY;
                                Graphic.StartPath(startX, startY);
                        //        Graphic.StartPath(tmpP.X * resoOutlineX, tmpY * resoOutlineY);
                                foreach (PointF aP in path)
                                {
                                    tmpY = (adjustedImage.Height - 1) - aP.Y;
                                    Graphic.AddLine(aP.X * resoOutlineX, tmpY * resoOutlineY);
                                }
                                tmpY = (adjustedImage.Height - 1) - tmpP.Y;
                                Graphic.AddLine(startX, startY);
                                Graphic.StopPath();
                            }
                        }
                        shrink = 0.4f;// 0.8f;          // shrink value in pixels!
                  ...
                  //      Gcode.Comment(finalString, string.Format("{0} Id=\"{1}\" Geometry=\"Fill\" PenColor=\"{2}\" PenWidth=\"{3:0.00}\">", XmlMarker.FigureStart, pathCount, ColorTranslator.ToHtml(Colors.MyPalette[index].Col).Substring(1), penWidth));
                        ...
                        {
                            while (colorMap[key][y].Count > 1)          // start at line 0 and check line by line
                                DrawColorMapGraphic(resol, key, y, 0, true);
                        }
                        //    Gcode.Comment(finalString, string.Format("{0}>", XmlMarker.FillEnd));
                   //     Gcode.Comment(finalString, string.Format("{0}>", XmlMarker.FigureEnd));
                    }
       ...
       //     Graphic.CreateGCode();      	// result is saved as stringbuilder in Graphic.GCode;
        ...
        private void ConvertColorMapBW(float resol)
        {
            ...
            for (int index = 0; index <= 255; index++)  // go through possible gray values
            {
                ...
                if (colorMap.ContainsKey(key) && (GrayValueMap[index].Use))
                {
                    ...
                    //    Gcode.Comment(finalString, string.Format("{0} Id=\"{1}\" PenColor=\"{2}\" PenName=\"{3}\" PenWidth=\"{4:0.00}\">", XmlMarker.FigureStart, (++pathCount), ColorTranslator.ToHtml(GroupColor.FromArgb(key,key,key)).Substring(1), key, resol));        //toolTable.indexName()));
        ...
        private void DrawColorMap(float reso, int toolNr, int line, int startIndex, bool first)
        {
            ...
            if (colorMap[toolNr][line].Count > 1)   // at least two numbers needed: start, stop
            {
                ...
                if (line < (resultImage.Height - factor - 1))
                {
                    ...
                    if (end)
                    { Gcode.PenUp(finalString, "PU"); } // end 1
                }
                else
                    Gcode.PenUp(finalString, "PU"); // end 2
            }
        }
        ...
        /// check recursive line by line for same color near by, by given x-value
        /// </summary>
        private void DrawColorMapGraphic(float reso, int toolNr, int line, int startIndex, bool first)
        {
            int start, stop, newIndex;
            float coordY = reso * (float)line;
            int factor = resoFactorX;
            ...
            if (colorMap[toolNr][line].Count > 1)   // at least two numbers needed: start, stop
            {
                ...
                if ((start >= 0) && (stop >= 0) && (cBGCodeFill.Checked))
                {
                    ...
                    if (first)                                              // is this inital first call?
                    {
                        Graphic.StartPath(coordX, coordY);
                        //    Gcode.MoveToRapid(finalString, coordX, coordY, "1st pos.");     // move to start pos
                        //    Gcode.PenDown(finalString, "PD");
                        first = false;
                    }
                    else
                        Graphic.AddLine(coordX, coordY);
                    //    Gcode.MoveTo(finalString, coordX, coordY, "");          // move to start pos
                    coordX = reso * (float)stop - tmpShrink;
                  //  Gcode.MoveTo(finalString, coordX, coordY, "");              // move to end pos
                    Graphic.AddLine(coordX, coordY);
                ...
                if (line < (resultImage.Height - factor - 1))
                {
                    ...
                    if (start < stop)                                   // comming from left, search from rigth to left
                    {
                        for (int k = stop + sfactor; k >= start - sfactor; k--)
                        {
                            ...
                            if (newIndex >= 0)                          // entry found
                            {
                                DrawColorMapGraphic(reso, toolNr, line + factor, newIndex, first);  // go on with next line
                                end = false;
                                break;
                            }
                        }
