...
// from https://github.com/XuechaoLee/PoTrace-1
// https://potrace.sourceforge.net/potrace.pdf
// https://potrace.sourceforge.net/potracelib.pdf
// 2024-09-27 l:643 add BitMapToBinaryAlpha with alphaCount, to detect if alpha-channel was set
...
namespace CsPotrace
{
    ...
    public partial class Potrace
    {
        ...
        public static bool[,] BitMapToBinaryFrom8bpp(Bitmap b, int _treshold)
        {
            BitmapData SourceData = b.LockBits(new Rectangle(0, 0, b.Width, b.Height), System.Drawing.Imaging.ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format8bppIndexed);
            // inflated Result
            ...
            unsafe
            {
                ...
                {
                    for (int x = 0; x < W; x++)
                        if (SourcePtr[x + Ydisp] < _treshold)
                            Result[x + 1, y + 1] = false;
                        else
                            Result[x + 1, y + 1] = true;
                    Ydisp = Ydisp + SourceStride;
        ...
        public static bool[,] BitMapToBinary1Bit(Bitmap b)//, int _treshold)
        {
            BitmapData SourceData = b.LockBits(new Rectangle(0, 0, b.Width, b.Height), System.Drawing.Imaging.ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format1bppIndexed);
            // inflated Result
            ...
            unsafe
            {
                ...
                for (int y = 0; y < H; y++)
                {
                    for (int x = 0; x < W; x++)
                    {
                        Result[x + 1, y + 1] = (SourcePtr[x / 8 + Ydisp] & masks[x % 8])>0;
                    }
                    Ydisp = Ydisp + SourceStride;
                }
            }
        ...
        public static bool[,] BitMapToBinaryColor(Bitmap b, Color findColor)
        {
            BitmapData SourceData = b.LockBits(new Rectangle(0, 0, b.Width, b.Height), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
            // inflated Result
            ...
            unsafe
            {
                ...
                for (int y = 0; y < H; y++)
                {
                    for (int x = 0; x < W; x++)
                    {
                        cb = SourcePtr[x * 3 + 0 + Ydisp];
                        cg = SourcePtr[x * 3 + 1 + Ydisp];
                        cr = SourcePtr[x * 3 + 2 + Ydisp];

                        if (Color.FromArgb(cr, cg, cb) == findColor)
                            Result[x + 1, y + 1] = false;
                        else
                            Result[x + 1, y + 1] = true;
                    }
        ...
        public static bool[,] BitMapToBinaryAlpha(Bitmap b, int _treshold)
        {

            BitmapData SourceData = b.LockBits(new Rectangle(0, 0, b.Width, b.Height), System.Drawing.Imaging.ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format32bppRgb);
            // inflated Result
            ...
            unsafe
            {
                ...
                {
                    for (int x = 0; x < W; x++)
                        if (SourcePtr[x * 4 + 3 + Ydisp] < _treshold)
                        { Result[x + 1, y + 1] = false; alphaCount++; }
                        else
                        { Result[x + 1, y + 1] = true; }
                    Ydisp = Ydisp + SourceStride;
                }
        ...
        public static Bitmap BinaryToBitmap(bool[,] Matrix, bool IgnoreBorder)
        {
            ...
            BitmapData CopyData = OutPutImage.LockBits(new Rectangle(0, 0, OutPutImage.Width, OutPutImage.Height), ImageLockMode.ReadWrite, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
        ...
        static Path findpath(bool[,] Matrix, IntPoint Start)
        {
            ...
            {
                // area += x * diry;
                //    if ((x > 0) && (y > 0) && (x < Matrix.GetUpperBound(0) - 1) && (y < Matrix.GetUpperBound(1) - 1))
