...
<g>", Width, Height);
        ...
        public static void Export2Graphic(ArrayList Fig, int Width, int Height)
        {
            foreach (ArrayList Path in Fig)
            {
                bool isContour = true;
                for (int i = 0; i < Path.Count; i++)
                {
                    Potrace.Curve[] Curves = (Potrace.Curve[])Path[i];
                    if (isContour)
                        GetContourPathGraphic(Curves);
                    else
                        GetHolePathGraphic(Curves);
                    isContour = false;
                }
            }
        }
        ...
        private static void GetContourPathGraphic(Potrace.Curve[] Curves)
        {
            for (int i = 0; i < Curves.Length; i++)
            {
                Potrace.Curve Curve = Curves[i];
                ...
                if (i == 0)
                {
                    Graphic.StartPath(Curve.A.X, Curve.A.Y);
                }

                if (Curve.Kind == Potrace.CurveKind.Bezier)
                {
                    ImportMath.CalcCubicBezier(p(Curve.A), p(Curve.ControlPointA), p(Curve.ControlPointB), p(Curve.B), GraphAddLine, "");
                }
                if (Curve.Kind == Potrace.CurveKind.Line)
                {
                    Graphic.AddLine(Curve.B.X, Curve.B.Y);
        ...
        private static void GetHolePathGraphic(Potrace.Curve[] Curves)
        {
            for (int i = Curves.Length - 1; i >= 0; i--)
            {
                Potrace.Curve Curve = Curves[i];
                if (i == Curves.Length - 1)
                {
                    Graphic.StartPath(Curve.B.X, Curve.B.Y);
                }

                if (Curve.Kind == Potrace.CurveKind.Bezier)
                {
                    ImportMath.CalcCubicBezier(p(Curve.B), p(Curve.ControlPointB), p(Curve.ControlPointA), p(Curve.A), GraphAddLine, "");
                }
                if (Curve.Kind == Potrace.CurveKind.Line)
                {
                    Graphic.AddLine(Curve.B.X, Curve.B.Y);
        ...
        private static void GraphAddLine(System.Windows.Point orig, string cmt)
        {
            Graphic.AddLine(orig.X, orig.Y);
        }

        private static System.Windows.Point p(DoublePoint a)
